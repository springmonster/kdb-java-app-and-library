...
public class c {
  ...
  int rBuffPos;
  /**
   * {@code wBuff} is the buffer used to store the outgoing message bytes when serializing an
   * object
   */
  byte[] wBuff;
  /**
  ...
  public static class Month implements Comparable<Month> {
     ...
     * of months since the beginning of the millennium. Post-milieu is positive and pre is
     * negative)
     *
  ...
  public static class Timespan implements Comparable<Timespan> {
    ...
    public String toString() {
      ...
      return s + i2((int) ((jj % 86400000000000L) / 3600000000000L)) + ":" + i2(
          (int) ((jj % 3600000000000L) / 60000000000L)) + ":" + i2(
          (int) ((jj % 60000000000L) / 1000000000L)) + "." + i9((int) (jj % 1000000000L));
    }
  ...
  Timestamp rp() {
    ...
    Timestamp p = new Timestamp(
        timeAsLong == nj ? timeAsLong : gl(MILLS_BETWEEN_1970_2000 + 1000 * d));
    if (timeAsLong != nj) {
  ...
  void w(Timestamp p) {
    ...
    w(millsSince1970 == nj ? millsSince1970
...
//object.getClass().isArray()   t(int[]) is .5 isarray is .1 lookup .05
  ...
  public static int n(final Object x) throws UnsupportedEncodingException {
    ...
    return x instanceof char[] ? new String((char[]) x).getBytes(encoding).length
        : Array.getLength(x);
  }
   ...
   * @param zip     true if to attempt compress serialised output (given uncompressed serialized
   *                data also has a length greater than 2000 bytes and connection is not localhost)
  ...
  public interface MsgHandler {
     ...
     * @throws IOException Thrown when message type is unexpected (i.e isnt a sync or async
     *                     message)
     */
   ...
   * order to reach the response message. If the caller has already indicated via
   * {@code setCollectResponseAsync} that the response message will be read async, later, then
   * return without trying to read any messages at this point; the caller can collect(read) the
   * response message by calling readMsg();
   *
   ...
   */
  public static final Object[] NULL = {null, Boolean.valueOf(false), new UUID(0, 0), null,
      Byte.valueOf((byte) 0), Short.valueOf(Short.MIN_VALUE), Integer.valueOf(ni), Long.valueOf(nj),
      Float.valueOf((float) nf), Double.valueOf(nf), Character.valueOf(' '), "",
      new Timestamp(nj), new Month(ni), new Date(nj), new java.util.Date(nj), new Timespan(nj),
   ...
   *          representation of the type (e.g. null would populate an empty string if x was an array
   *          of Strings)
   */
   ...
   * <p>
   * A keyed table(a.k.a. Flip) is a dictionary where both key and value are tables themselves. For
   * ease of processing, this method, td, table from dictionary, can be used to remove the key.
   * </p>
   *
